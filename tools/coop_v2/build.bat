...
cl.exe /nologo /W4 /O2 /GS /sdl /guard:cf /MT /EHsc /LD /DUNICODE /D_UNICODE /D_CRT_SECURE_NO_WARNINGS /I. coop_application.cpp coop_dll.cpp coop_runtime.cpp coop_netgame.cpp player2.cpp save_sync.cpp window_hook.cpp ServerClient\MClient.cpp ServerClient\MClientONLINE.cpp ServerClient\MServer.cpp ServerClient\MServerONLINE.cpp ServerClient\MStandalone.cpp ServerClient\MTypes.cpp ServerClient\SteamManager.cpp /Fo:build\ /Fe:build\coop_dll.dll /link /DEF:coop_dll.def /LIBPATH:"%~dp0SteamWorksSDK" GameNetworkingSockets.lib steam_api.lib bcrypt.lib user32.lib /PDB:build\coop_dll.pdb
if errorlevel 1 exit /b 1
