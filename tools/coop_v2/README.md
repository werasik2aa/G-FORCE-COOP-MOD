...
## Открытый игровой backlog — 2026-09-13

Это список для исследования и двухпроцессного теста, а не список уже
реализованных функций. Метки имеют строгий смысл: **наблюдение** подтверждено в
игре пользователем, **гипотеза** — только направление RE, а **не проверено** не
разрешает считать проблему исправленной после сборки.

| Приоритет | Задача | Статус и требуемое доказательство |
| --- | --- | --- |
| P0 | Смерть/respawn может вернуть игрока в начало локации с уже закрытыми дверями. | **Наблюдение.** Разделить outer `DeathMode`, checkpoint/load и переход уровня; снять лог host и client от получения урона до respawn. Не подменять checkpoint или телепорт без найденного native маршрута. |
| P0 | После отдельных катсцен погибает один или оба игрока. | **Наблюдение.** Нужен воспроизводимый сейв/сцена и лог перехода cutscene → gameplay → death. Это не то же самое, что Ledge recovery или обычная смерть. |
| P0 | Подключение игрока после уже пройденных trigger/event. | **Главный открытый случай.** Late-join peer должен получить persistent последствия уже пройденного пути: состояние дверей/задвижек, counters, spawned/despawned objects, нужный progression. Нельзя слепо повторять все старые event: transient hit, cutscene и non-idempotent spawn могут дать двойной объект или повторную сцену. Нужен catalog durable world-state и явная политика snapshot/replay по root object/cell. |
...
| P2 | Добавить чат. | Отдельная feature-задача: сначала определить transport, лимиты, ник/peer identity, UI focus и поведение во время menu/cutscene. Она не должна использовать игровые input-edge пакеты. |

### Рекомендуемый порядок проверки

1. Late join после уже пройденных trigger/event: сначала определить persistent
   state и запрещённые для replay transient routes.
2. Сейв/respawn и смертельные cutscene-переходы: это может блокировать весь
   прогресс и искажать остальные тесты.
3. Конкретные закрывающие путь двери/задвижки и key-card как физический объект.
4. Сейберизация вместе с ошибочным HUD; затем хлыст и лазерные мины Мухи на
   одиночных проверяемых объектах.
5. Streaming-переходы на альтернативных путях, затем свежий уровень для
   spawn/kill/attack NPC и только после этого дизайн чата.

Для каждого нового случая сохранить оба runtime-лога с PID, название/место
