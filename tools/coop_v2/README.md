...
- `F7` — открыть standalone-сервер `0.0.0.0:44139` и Steam P2P listen socket на
  виртуальном порту `44140`.
- `F8` — подключить текущий процесс к `127.0.0.1:44139`.
...
1. В обоих процессах дождаться штатного спавна P1.
2. В окне host нажать `F7`, в окне client — `F8`.
...
Поэтому в сборку добавлен одноразовый диагностический дамп: `DumpActionBindingsOnce()`
из `GameTick()` один раз пишет в `gforce_coop.log` живую таблицу биндов P1 (`n`, тип,
scan-код с именем клавиши) и `[pad + 0x278C]`. Это не кадровый лог — один проход по
`0x43` записям за сессию, с повтором только если пад ещё не зарегистрирован
(`0x487F10`). Рядом с ним `CaptureLocalPress` пишет по одной строке
`[press] first local press n=..` на каждый индекс за процесс (максимум 68 строк), то
есть одно нажатие клавиши сразу называет свой индекс.
