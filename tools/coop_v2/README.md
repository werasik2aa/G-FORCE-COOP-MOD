...
| P1 | Спавн NPC, анимации, атаки и remote damage. | HP host → client уже имеет отдельный маршрут, но полный результат **не проверен**. Проверить delayed spawn, немедленное убийство, attack animation и урон в обоих направлениях на свежем уровне; не объявлять AI синхронизированным по одному удачному бою. |
...
1. Сейв/respawn и смертельные cutscene-переходы: это может блокировать весь
   прогресс и искажать остальные тесты.
2. Конкретные закрывающие путь двери/задвижки и key-card как физический объект.
3. Сейберизация вместе с ошибочным HUD; затем хлыст и лазерные мины Мухи на
   одиночных проверяемых объектах.
4. Свежий уровень для spawn/kill/attack NPC и только после этого дизайн чата.
