...
private:
	...
	typedef HRESULT (WINAPI* D3DCreateDeviceFn)(
		IDirect3D9*, UINT, D3DDEVTYPE, HWND, DWORD,
		D3DPRESENT_PARAMETERS*, IDirect3DDevice9**);
	typedef HRESULT (WINAPI* D3DResetFn)(
	...
	void ConfigureWindowedPresentation(
	...
	HRESULT Present(IDirect3DDevice9* device, const RECT* source_rectangle,
		const RECT* destination_rectangle, HWND destination_window,
	...
	LRESULT WindowProcedure(HWND window, UINT message, WPARAM w_param,
		LPARAM l_param);

	static HRESULT WINAPI HookD3DReset(
		IDirect3DDevice9* device, D3DPRESENT_PARAMETERS* parameters);
	static HRESULT WINAPI HookD3DCreateDevice(
		IDirect3D9* direct3d, UINT adapter, D3DDEVTYPE device_type,
		HWND focus_window, DWORD behavior_flags,
		D3DPRESENT_PARAMETERS* parameters, IDirect3DDevice9** device);
	static IDirect3D9* WINAPI HookDirect3DCreate9(UINT sdk_version);
	static HRESULT WINAPI HookD3DPresent(IDirect3DDevice9* device,
	...
	static LRESULT CALLBACK HookWindowProcedure(HWND window, UINT message,
