...
| Mooch | Остаётся единственным shared объектом. Сохраняется transform/presentation синхронизация без remote Fly controller/input/camera takeover: receiver берёт `position`, pitch/bank из owner transform и заменяет только horizontal rotation.y подтверждённым owner camera yaw. Только local owner тикает native Fly и публикует yaw после этого tick; receiver очищает remote active-control flag. `Fly_Deactivated` имеет authority только у local owner: он публикует новый zero-owner snapshot. Dual laser — отдельный reliable event: receiver временно подставляет ray из центра локальной копии Мухи и запускает зарегистрированный `Fly_Active::Update` с exact raw-button edge, не выбирая Fly mode и не забирая HUD/camera. Shadow pass возвращает также полный Fly camera request/apply window `handler+0x91C..+0x9B7`, иначе один кадр уводит камеру P1 к Мухе. Direct item pulse допускается только как явно залогированный fallback. Carry остаётся future route. | `gameplay/fly/` |
